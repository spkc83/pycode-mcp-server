...
def extract_raises(docstring: Optional[str]) -> List[Dict[str, str]]:
    ...
    sphinx_pattern = re.compile(r":raises?\s+(\w+(?:\.\w+)*)\s*:\s*(.+)", re.MULTILINE)
    ...
    section_pattern = re.compile(r"^\s*(?:Raises|Exceptions)\s*:?\s*$", re.MULTILINE)
    ...
    if match:
        ...
        entry_pattern = re.compile(r"^\s{4}(\w+(?:\.\w+)*)(?:\s*:\s*(.+?))?$", re.MULTILINE)
        ...
        if len(related) < 5:
            for member_name, member in inspect.getmembers(module):
                ...
                if callable(member) and callable(obj):
                    if (
                        obj_name.lower() in member_name.lower()
                        or member_name.lower() in obj_name.lower()
                    ):
    ...
    if inspect.isclass(obj):
        ...
        for member_name, member in inspect.getmembers(obj):
            if member_name.startswith("_") and not member_name.startswith("__"):
                continue
            if member_name.startswith("__") and member_name not in (
                "__init__",
                "__call__",
                "__enter__",
                "__exit__",
        ...
        if hasattr(obj, "__annotations__"):
            for attr_name, attr_type in obj.__annotations__.items():
                if not attr_name.startswith("_"):
                    result["attributes"].append({"name": attr_name, "type": str(attr_type)})
...
def _get_jedi_structured_docs(name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            sigs = sig_script.get_signatures(
            ...
            if sigs:
                ...
                for p in sig.params:
                    ...
                    param_info["kind"] = (
                        "keyword_only" if desc and "keyword" in desc else "positional_or_keyword"
                    )
        ...
        try:
            if jedi_name.module_name:
                search_results = jedi.Script(f"import {jedi_name.module_name}").complete(
                    1, len(f"import {jedi_name.module_name}") + 1
                )
                obj_name = parts[-1]
                related = [
                    r.name
            ...
            try:
                ...
                for c in completions:
                    if c.type in ("function", "method"):
                        if c.name.startswith("__") and c.name not in (
                            "__init__",
                            "__call__",
                            "__enter__",
                            "__exit__",
        ...
        if jedi_name.type == "module":
            try:
                ...
                result["exports"] = [c.name for c in completions[:20] if not c.name.startswith("_")]
            except Exception:
                pass
...
def get_local_docs(
    name: str, use_cache: bool = True, structured: bool = True
...
def main() -> None:
    ...
    parser.add_argument(
        "--raw", action="store_true", help="Return raw pydoc text instead of structured JSON"
    )
