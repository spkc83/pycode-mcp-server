        ...
        if files:
            ...
            for f in files:
                ...
                if (
                    ...
                    and not parts[0].endswith(".egg-info")
                ):
...
def main() -> None:
    ...
    parser.add_argument(
        "--simple", action="store_true", help="Simple output: just name and version"
    )
    parser.add_argument("--package", "-p", type=str, help="Get details for a specific package")
    parser.add_argument(
        "--find-import", type=str, help="Find which package provides an import name"
    )
    ...
    if args.find_import:
        ...
        if pkg:
            ...
            print(
                json.dumps({"error": f"No package found for import '{args.find_import}'"}, indent=2)
            )
        return
